...
import { IDiscount, IFindDiscount, IPromotion, ETypePromotion } from './IDiscount.interface';
...
export class DiscountsService {
    ...
    async findDiscounts( infoProducts: IFindDiscount[] ): Promise<IPromotion[]> {
        ...
        const discountsByBrands:Map<string, IDiscount> = await this.findByBrands(brands)
        ...
        for (const infoProduct of infoProducts) {
            ...
            if(infoProduct.amount >= discount.threshold && bestDiscount.totalDiscount < discount.discount) {
            ...
            if( discount.discount > bestOffer.totalDiscount ) {
                ...
                bestOffer.threshold = discount.threshold
        ...
        let resp:IPromotion[] = [];
        console.log(bestDiscount.totalDiscount>0, bestDiscount.totalDiscount)
        console.log(bestOffer.totalDiscount>0 && bestOffer.totalDiscount> bestDiscount.totalDiscount, )
        if(bestDiscount.totalDiscount>0) resp.push(bestDiscount)
        if(bestOffer.totalDiscount>0 && bestOffer.totalDiscount> bestDiscount.totalDiscount) resp.push(bestOffer)
        return resp
    }

    private async findByBrands(brands: string[]): Promise< Map<string, IDiscount>>{
        let map: Map<string, IDiscount> = new Map()
        const discountsFilter: IDiscount[] = await this.discountModel.find({ brand: { "$in" : brands} }).exec();
